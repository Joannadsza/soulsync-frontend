...
export function Header() {
  ...
  const publicNavItems = [
    ...
    { href: "/about", label: "About SoulSync" },
  ];

  const authNavItems = [
    { href: "/dashboard", label: "Dashboard", icon: <LayoutDashboard className="w-4 h-4" /> },
    { href: "/progress", label: "Progress", icon: <TrendingUp className="w-4 h-4" /> },
    { href: "/find-therapist", label: "Find Therapist", icon: <Stethoscope className="w-4 h-4" /> },
  ];
        ...
        <div className="flex h-16 items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <nav className="hidden md:flex items-center space-x-1">
              ...
              {/* Auth nav items — shown when logged in */}
              {isAuthenticated &&
                authNavItems.map((item) => (
                  <Link
                    key={item.href}
                    href={item.href}
                    className="px-4 py-2 text-sm font-medium text-muted-foreground hover:text-foreground transition-colors relative group flex items-center gap-1.5"
                  >
                    {item.icon}
                    {item.label}
                    <span className="absolute bottom-0 left-0 w-full h-0.5 bg-primary scale-x-0 group-hover:scale-x-100 transition-transform duration-200 origin-left" />
